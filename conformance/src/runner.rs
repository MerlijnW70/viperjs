...
impl Runner {
    ...
    fn run_once(
        ...
        // §25.4.3.14's `AgentCanSuspend()` asks §9.7's `[[CanBlock]]`, which is the **host's** to
        // answer, and this host answers *true* — on the agent running the file as much as on the
...
///
/// What is absent stays absent — `gc` and `IsHTMLDDA` — so a test that needs one fails saying so
...
pub fn install_host(heap: &mut Heap, realm: &viperjs::realm::Realm) -> viperjs::heap::ObjectId {
    ...
    // API that is depends on the thread rather than on the realm — see [`crate::agent::attach`].
...
fn evaluate(
    ...
    // a test which failed to parse — and therefore never reached an end — is cleaned up too.
    crate::agent::forget();
    ...
    // §9.7's `[[CanBlock]]` — see the flag check in `run_once` for why this host answers true, and
...
var r = /(?i:a)/;",
    ...
    fn what_this_harness_cannot_honestly_run_is_skipped_and_says_which() {
        ...
        // …and the two it still cannot do are *absent* rather than stubbed, so a test that needs
...
---*/
        ...
        // `Atomics.wait` existed to be tested — and what stops the pair silently *swapping* again,
        ...
        // …and this is the host's actual §9.7 answer, which the two rows above only *describe*.
        ...
        // is exactly this program — a wait whose value has already changed, made only to see
        // whether the call throws — and every one of the 109 files that starts an agent runs it.
        let waiting = run(
            &root,
            "/*---\ndescription: blocks\n---*/\n\

...
//!
//! Every test here runs *source* — parsed, compiled and executed — rather than asserting on a
//! chunk. That is deliberate: an instruction sequence is an implementation detail and a value is
//! not, so a test written this way survives the compiler changing its mind about how to emit
//! something. The exceptions are the ones about a malformed chunk, which no compiler emits and
//! which are therefore built by hand.

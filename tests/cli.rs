//! The `viper` binary, run as a process.
//!
...
//! `src/bin/viper.rs`. What is left is the acting: reading a file, printing a completion value or
...
fn viper(arguments: &[&str], stdin: &str) -> (String, String, Option<i32>) {
    let mut child = Command::new(env!("CARGO_BIN_EXE_viper"))
        .args(arguments)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
...
fn a_multi_line_file_runs_as_one_script() {
    ...
    let directory = std::env::temp_dir().join("viper-cli-test");
    ...
    let (out, err, status) = viper(&[path.to_str().expect("a path")], "");
...
fn the_exit_status_tells_a_shell_which_kind_of_wrong_it_was() {
    ...
    let (_, err, status) = viper(&["-e", "throw new TypeError('nope')"], "");
    assert_eq!(status, Some(1));
    assert!(err.contains("TypeError: nope"), "{err}");

    let (_, err, status) = viper(&["-e", "var x = ;"], "");
    assert_eq!(status, Some(1));
    assert!(err.contains("SyntaxError"), "{err}");

    let (_, err, status) = viper(&["--nope"], "");
    assert_eq!(status, Some(2));
    assert!(err.contains("unknown option"), "{err}");

    let (_, err, status) = viper(&["no-such-file-anywhere.js"], "");
    assert_eq!(status, Some(2));
...
fn a_time_budget_stops_a_loop_the_script_tries_to_catch() {
    ...
    let (out, _, status) = viper(&["--time-budget", "10000", "-e", "1 + 1"], "");
    assert_eq!(out, "2\n");
    assert_eq!(status, Some(0));
}
...
fn help_and_version_answer_on_stdout_and_succeed() {
    ...
    let (out, err, status) = viper(&["--help"], "");
    assert!(out.contains("usage:"), "{out}");
    assert_eq!(err, "");
    assert_eq!(status, Some(0));

    let (out, _, status) = viper(&["-V"], "");
    assert!(out.starts_with("viper "), "{out}");
    assert_eq!(status, Some(0));
}

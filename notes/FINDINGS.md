# FINDINGS — what each area cost, and what it taught

Per-area write-ups, moved out of [AGENTS.md](../AGENTS.md) so they are not loaded into every
session. **Read the section for the area you are about to touch**; each records what a slice cost
beyond its clause, which comment lied, and which hypothesis died on the way.

Nothing here is optional reading once you are *in* an area — a good half of these sections exist
because somebody re-derived a dead end that was already written down. The index is the fastest way
in.
...
- [An array index is a key of its own, and the biggest win was in the file nobody costed](#an-array-index-is-a-key-of-its-own-and-the-biggest-win-was-in-the-file-nobody-costed)
- [§25.4's three waits do not share a fate, and one of them works with a single agent](#254s-three-waits-do-not-share-a-fate-and-one-of-them-works-with-a-single-agent)
- [Duplicate named capture groups, and the check that needs *which* disjunction](#duplicate-named-capture-groups-and-the-check-that-needs-which-disjunction)
- [One `?`, and 109 runs: §15.8.4 rejects where the generator clauses throw](#one--and-109-runs-1584-rejects-where-the-generator-clauses-throw)
- [§10.4.5's internal methods, and the three comments that stated the bug as a rule](#1045s-internal-methods-and-the-three-comments-that-stated-the-bug-as-a-rule)
- [A lookahead is quantifiable, and Annex B says exactly where](#a-lookahead-is-quantifiable-and-annex-b-says-exactly-where)
- [§B.1.2 is the other half of Annex B, and DR-0008 had already stopped covering it](#b12-is-the-other-half-of-annex-b-and-dr-0008-had-already-stopped-covering-it)
- [Two of §20.2.1.1's four kinds were missing, and the lookup answered with the wrong callable](#two-of-20211s-four-kinds-were-missing-and-the-lookup-answered-with-the-wrong-callable)
- [`arguments` was not iterable, and the bucket that found it was three layers away](#arguments-was-not-iterable-and-the-bucket-that-found-it-was-three-layers-away)
- [Seven shipped library functions were simply absent, and `typeof` found them in one line](#seven-shipped-library-functions-were-simply-absent-and-typeof-found-them-in-one-line)
- [A step that throws is a step that finished, and destructuring was closing anyway](#a-step-that-throws-is-a-step-that-finished-and-destructuring-was-closing-anyway)
- [§9.1.1.2.5 is four steps, and a `with` write was doing one of them](#91125-is-four-steps-and-a-with-write-was-doing-one-of-them)
- [§27.2.4.1 step 8.a was missing from all four combinators](#27241-step-8a-was-missing-from-all-four-combinators)
- [§B.2.2's four helpers went round the internal methods instead of through them](#b22s-four-helpers-went-round-the-internal-methods-instead-of-through-them)
- [A class stopped being a predicate on one code point](#a-class-stopped-being-a-predicate-on-one-code-point)
- [§23.2.5.1's step 7 and step 8 are alternatives, and ViperJS ran both](#23251s-step-7-and-step-8-are-alternatives-and-viperjs-ran-both)
- [The shrink half, and a read that asked the buffer instead of the window](#the-shrink-half-and-a-read-that-asked-the-buffer-instead-of-the-window)
- [`api.rs` exists — and what an embedder could not do before it](#apirs-exists--and-what-an-embedder-could-not-do-before-it)
- [A run has a time budget, and it cannot be caught — DR-0022](#a-run-has-a-time-budget-and-it-cannot-be-caught--dr-0022)
- [The harness's own reach is part of the measurement, and it was hiding 68 runs](#the-harnesss-own-reach-is-part-of-the-measurement-and-it-was-hiding-68-runs)
- [81.26% to 82.16% in seven slices, six of which a comment told on](#8126-to-8216-in-seven-slices-six-of-which-a-comment-told-on)
